...
F2 completed the protocol surface those features had left open, still inside this crate and still as rules rather than as adapter code: `lsp_core::intentions` owns the Alt+Enter list — two `codeAction` requests (one scoped to the diagnostics under the caret, one to the range), merged, grouped quickfix/refactor/source, preferred-first within a group, and deduplicated by title and kind — plus which diagnostics make Organize Imports worth offering as a quick fix; `lsp_core::signature_help` owns both parameter-label shapes, the overload rule where a signature's own `activeParameter` beats the response-level one, and the caret-to-argument arithmetic that a server's answer cannot supply because the caret has moved by the time it lands; `lsp_core::document_highlight` and `lsp_core::inlay_hint` own their response shapes, the latter with the line-range request that keeps a large file affordable.
...
- Done in L2: servers start lazily, on the first opened file of a language that has an enabled entry — not at app launch, which would spawn a dozen processes for a project that uses one language. `catalog::language_id_for_path` was the extension-to-language-id table that decision needed, deliberately separate from `syntax-core`'s grammar detection because these are the identifiers the protocol defines. That separation turned out to be the wrong seam — it duplicated file detection and silently drifted — and was replaced by [ADR-0018](0018-single-source-language-detection.md): detection comes from `syntax-core`, `lsp-core` keeps only the protocol id divergences and the server commands.
