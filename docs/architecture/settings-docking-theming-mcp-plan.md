...
| M6 | Real MCP protocol surface | `initialize`, `tools/list` with hand-written schemas, `tools/call`, notification handling (no body, `202`); one shared `dispatch_method` behind both `tools/call` and the flat method names; `POST /mcp` alias ([ADR-0012](decisions/0012-mcp-protocol-index-and-lifecycle.md)) | `cargo test -p mcp-server`; an off-the-shelf MCP client completes the handshake and lists the tools |
| M7 | Index tools | `mcp_server::start` takes the `SearchModel`'s `Arc<RwLock<IndexSlot>>`; `index_status`, `search_text`, `find_files`, `find_definitions`, `find_usages`, `find_implementations`, `find_supertypes`, `resolve_declaration`, `replace_in_files`, each on a blocking worker; `resolve_replacements` moved into `index-core`; new `AppSession::content_for_path` + `EditorCommand::BufferContentForPath` | `cargo test -p mcp-server -p index-core -p app-core` (index tools tested against a real `TextIndex`); an agent's search results match Find in Files |
| M8 | MCP settings + lifecycle | `Settings::mcp_enabled`/`mcp_port`; `applyMcpSettings` replacing `startMcpServer` (stop, re-read, restart) with `mcpStarted`/`mcpStopped`/`mcpFailed`; `shutdownMcpServer` on window close; discovery file removed on shutdown; Settings > MCP page (enable, port with `0` = Automatic, live status, discovery path) | `cargo test -p app-config`; Xvfb: pin a port and confirm the discovery file, toggle off and confirm the port stops answering, toggle on again — all without restarting |
