...
| G1 | `index-core` text-index skeleton | tantivy ngram(3) content field + `grep-searcher`/`grep-regex` verification; initial walk via `ignore` crate; incremental updates via `ProjectWatcher` (shipped as `reindex_file`/`remove_file`, but left uncalled — the watcher was actually wired in ADR-0010's task T6) | `cargo test -p index-core`: build+query round trip on a fixture repo |
