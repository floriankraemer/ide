...
//!
//! Both protocols this IDE speaks over a child process's stdio frame their
//! messages this way: the Language Server Protocol's base protocol and the
//! Debug Adapter Protocol's. The two are not merely similar, they are the
//! same bytes, so this lives in one crate rather than being written twice
//! (ADR-0041) — `lsp-core` and `dap-core` both read and write through it.
//!
...
/// making us allocate arbitrarily. Payloads are JSON documents; 64 MiB is far
/// beyond anything a language server or debug adapter legitimately sends.
